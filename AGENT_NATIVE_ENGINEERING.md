...
跨 Agent 调用存在两层事务边界：子 Agent 的工具事务与父 `task` 的保护快照。父边界不能因为“文件发生变化”
就回滚子 Agent 通过权威工具完成的合法提交，也不能因为 task 角色正确就整段放弃保护。正确做法是重新判定
闭环不变量：source state 保持同态、runtime identity 可认证、bundle/review hash 一致、PASS/FAIL/STOP 语义一致，
并只放行最小 state delta 与唯一登记工件。任何额外 JSON、脚本、既存证据改写或字段漂移仍逐字节回滚。

失败也必须按认识论类型分流。科学或审计 FAIL 是“证据否定了当前主张”，应保留同一 review state、写入
machine-readable remediation 并形成 INVALID+STOP；capability unavailable 是“没有能力作出判断”，只能形成
`BLOCKED_CAPABILITY`，不得伪装成 FAIL。把二者合并会同时污染科研结论与恢复路径。
...
管理员应在生命周期事件已经积累到“是否可以行动”的决策检查点调用。若与工作任务在最初同一批
fan-out，Flash 往往只会观察到 `CURRENT_STARTED`；这只是早期快照，不是最终完成证明。1–3 个简单
任务由 M3 直接等待；大量可选/过期/冲突事件混杂时才支付管理员调用成本。
